...
1. **Model Evaluation:** 
    1. How do we evaluate the performance of different models?  
    1. What metrics to use? <br/><br/>
    
We now demonstrate this process applied to a dataset of student first names from a previous offering. In this chapter, we proceed quickly in order to give the reader a general sense of a complete iteration through the lifecycle. In later chapters, we expand on each step in this process to develop a repertoire of skills and principles.
...
Let's begin by looking at our data.

```{admonition} "Attention to Process" 
...
```{admonition} Apply corrections 
We will study how to identify anomalies in data and apply corrections. The differences in capitalization will cause our programs to think that 'BRYAN' and 'Bryan' are different names when they are identical for our purposes. Let's convert all names to lower case to avoid this. 
```
